...
(defn component
  ...
  ([preloader-id {:keys [uri]}]
   (reagent/lifecycles {:component-will-mount #(a/dispatch [:x.app-core.load-handler/synchronize-loading!        preloader-id])
                        :reagent-render [:img {:on-load #(a/dispatch [:x.app-core.load-handler/->synchron-signal preloader-id])
                                               :src      (param uri)
                                               :style    {:display "none"}}]})))
