...
(ns dnd-kit.views
    (:require ["@dnd-kit/core"      :as dnd-kit.core]
              ...
              [dnd-kit.helpers      :as helpers]
              [dnd-kit.prototypes   :as prototypes]
...
(defn- render-items
  ...
  ; @param (keyword) sortable-id
  ; @param (map) sortable-props
  ; @param (strings in vector) item-order
  [sortable-id sortable-props item-order]
...
(defn- dnd-context
  ...
  (let [sensors (useSensors (useSensor PointerSensor)
       ...
       [DndContext {:sensors            sensors
                   ...
                   [SortableContext {:items    item-order
                                     :strategy rectSortingStrategy}
                                    [render-items sortable-id sortable-props item-order]]
...
(defn- sortable-body
  ...
  ; BUG#0047 (plugins/dnd_kit/helpers.cljs)
  ; BUG#0048 (plugins/dnd_kit/helpers.cljs)
  (let [item-order (helpers/get-item-order sortable-id sortable-props)]
       (if (-> item-order empty? not)
           [:<> [:f> dnd-context sortable-id sortable-props item-order]])))
