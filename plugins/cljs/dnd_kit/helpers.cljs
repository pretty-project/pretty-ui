...
(defn sortable-did-mount-f
  ...
  ; hanem csak azok azonosítóival kell dolgoznia, így a dnd-kit/body komponens számára átadott
  ; elemek bármilyen típusúak lehetnek és nem szükséges az :id vagy :namespace/id kulcsot tartalmazniuk,
  ; elegendő, ha az item-id-f függvénynek átadva őket egyedi azonosító nyerhető ki belőlük!
  (letfn [(f [state item]
...
(defn sortable-will-unmount-f
  ...
  ; mielőtt annak minden életciklusa rendben lezajlana (nem tudom, mi történik
  ; a dnd-kit.core névtérben) és a dnd-kit.core/DndContext React komponens
  ; a következő hibaüzenetet jelenítené meg a konzolon:
...
(defn get-item-order
  ...
  ;
  ; @example
  ; (get-item-order :my-sortable {:item-id-f :value
  ...
  [sortable-id {:keys [item-id-f items]}]
  ; BUG#0048
  ; Az elemek azonosítóinak sorrendje azután érhető el a state/SORTABLE-STATE atomban,
  ; miután a body komponens :component-did-mount életciklusa megtörtént.
  ; Annak érdekében, hogy az első rendereléskor ez ne okozzon villanást az elemek
  ; késve történő megjelenése miatt, a sortable-props térképből közvetlenül kinyert
  ; sorrend helyettesíti az elemek sorrendjét, amíg az nem elérhető az ... atomban.
  ;
  ; Ez egy bugfix és lehetséges, hogy jobb megoldás lenne, ha az elemek sorrendjének
  ; elérhetősége nem függne a body komponens életciklusaitól!
  (or (get-in @state/SORTABLE-STATE [sortable-id :item-order])
      (vector/->items items item-id-f)))
...
(defn drag-start-f
  ...
  ; BUG#0017 (plugins/cljs/dnd_kit/README.md)
