...
(defn items-different?
  ...
  ([a b]
   ...
   ; kulcsai szerint hasonlítja össze 'a' és 'b' térképet, tehát 'a' térképet
   ; tekinti a megváltozott elemnek és 'b' térképet az eredetiről készült másolatnak.
   (let [keys (keys a)]
  ...
  ([a b keys]
   ...
   ;     Pl.: 42 = "42"
   ;     Pl.: A szövegmezőkben megadott számokat a szerver integer típusra alakíthatja!
   (letfn [(f [key] (if (-> a key mixed/blank?)
                        (-> b key mixed/nonblank?)
                        (not= (-> a key str)
                              (-> b key str))))]
          (some f keys))))
