...
(defn backup-stored-value!
  ...
  (assoc-in db [:pretty-inputs :input-handler/backup-values input-id]
...
(defn reset-value!
  ...
  ; @return (map)
  [db [_ input-id {:keys [value-path]}]]
  (let [backup-value (get-in db [:pretty-inputs :input-handler/backup-values input-id])]
