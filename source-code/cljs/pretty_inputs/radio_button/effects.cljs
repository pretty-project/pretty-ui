...
(ns pretty-inputs.radio-button.effects
    (:require [pretty-inputs.radio-button.events :as radio-button.events]
...
(r/reg-event-fx :pretty-inputs.radio-button/button-did-mount
  ; @ignore
  ;
...
(r/reg-event-fx :pretty-inputs.radio-button/select-option!
  ; @ignore
  ;
  ; @param (keyword) button-id
  ; @param (map) button-props
  ...
  (fn [{:keys [db]} [_ button-id {:keys [on-select option-value-f] :as button-props} option]]
      (let [option-value (option-value-f option)]
           {:db       (r radio-button.events/select-option! db button-id button-props option)
            :fx       [:pretty-inputs.input/mark-input-as-visited! button-id]
            :dispatch [:pretty-build-kit/dispatch-event-handler! on-select option-value]})))

(r/reg-event-fx :pretty-inputs.radio-button/clear-value!
  ; @ignore
  ;
  ; @param (keyword) button-id
  ; @param (map) button-props
