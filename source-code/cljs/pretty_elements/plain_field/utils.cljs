...
(defn resolve-field-change-f
  ...
  ;   it's called with a delay and the field's content can change during that latency.
  (when (plain-field.env/type-ended? field-id)
        (r/dispatch-sync [:pretty-elements.plain-field/type-ended field-id field-props])))
