...
(ns pretty-elements.select.views
    (:require [fruits.hiccup.api                 :as hiccup]
              ...
              [pretty-elements.select.attributes :as select.attributes]
              [pretty-elements.select.env        :as select.env]
...
(defn- option-field
  ...
  (if extendable? (let [adornment-on-click [:pretty-elements.select/add-option! select-id select-props]
                        adornment-props    {:icon :add :on-click adornment-on-click :title :add!}]
                       [text-field.views/element :pretty-elements.select/option-field
...
(defn- select-options
  ...
  (if (input.env/popup-rendered? select-id)
      [:div {:class :pe-select--options}
            [pretty-layouts/struct-popup :pretty-elements.select/options
                                         (assoc popup :body     [select-options-body   select-id select-props]
                                                      :header   [select-options-header select-id select-props]
                                                      :on-cover {:fx [:pretty-elements.input/close-popup! select-id select-props]})]]))
...
(defn- select-button
  ...
  ; @param (map) select-props
  [select-id select-props]
  (let [on-click     {:fx [:pretty-elements.input/render-popup! select-id select-props]}
