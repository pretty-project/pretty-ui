...
(ns app-plugins.item-lister.download-handler.subs
...
(defn get-resolver-id
  ...
  (let [handler-key (r lister-handler.subs/get-meta-item db extension-id item-namespace :handler-key)]
...
(defn items-received?
  ...
  (let [items-received? (r lister-handler.subs/get-meta-item db extension-id item-namespace :items-received?)]
...
(defn get-all-item-count
  ...
  (let [all-item-count (r lister-handler.subs/get-meta-item db extension-id item-namespace :document-count)]
...
(defn no-items-received?
  ...
  ; @return (boolean)
  [db [_ extension-id item-namespace]]
  (let [received-count (r lister-handler.subs/get-meta-item db extension-id item-namespace :received-count)]
       (= received-count 0)))
