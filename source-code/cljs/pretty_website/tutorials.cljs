...
;
; @note
; In case the provided ID changes the item does not take the new ID, it keeps the first one that was provided.
; In case the element properties changes, the element does not generate a new ID.
; It only generates new ID when it gets rerendered.
;
; @note
; In case of no ID is provided and the item gets rerendered it does not generate a new ID for itself, it keeps the first one that was generated.
...
;
; Pretty UI items can display [multitype contents](https://github.com/mt-app-kit/cljc-multitype-content).
