...
(defn get-request-id
  ...
  ; van-e (az engine-synchronizing? függvénynek elegendő egy request-id azonosítót figyelnie).
  ;
...
(defn use-query-params
  ...
  ; XXX#7061
  ; Az egyes engine-ek {:query-params {...}} meta-adataként beállított térképbe
  ; az engine által beleírt adatok, minden az engine által küldött Pathom resolver
  ; és mutation lekérés resolver-props és mutation-props térképének alapját képezi.
  ;
