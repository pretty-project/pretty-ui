...
(defn handle-route!
  ...
  ;    a body komponens {:item-id "..."} paramétere, ...
  ...
  ;         a {:item-id "..."} paramétert. Ezért a szükséges a browse-item! függénynek átadni
  ;         a {:item-id "..."} paraméter értékét, hogy eltárolja az aktuálisan böngészett
  ...
  ;        a body komponens {:item-id "..."} paramétereként átadott azonosítót az aktuálisan
  ...
  (r core.events/browse-item! db browser-id (or (r routes.subs/get-derived-item-id db browser-id)             ; A)
                                                (r body.subs/get-body-prop         db browser-id :item-id)))) ; B)
