...
(defn reset-downloads!
  ...
  (let [items-path (r mount.subs/get-body-prop db extension-id item-namespace :items-path)]
       (-> db (dissoc-in items-path)
              (dissoc-in [:plugins :item-lister/meta-items extension-id :document-count])
              (dissoc-in [:plugins :item-lister/meta-items extension-id :received-count])
              (dissoc-in [:plugins :item-lister/meta-items extension-id :items-received?]))))
...
(defn items-received
  ...
  (assoc-in db [:plugins :item-lister/meta-items extension-id :items-received?] true))
...
(defn store-received-items!
  ...
  (let [resolver-id (r download.subs/get-resolver-id db extension-id item-namespace :get)
        items-path  (r mount.subs/get-body-prop      db extension-id item-namespace :items-path)
...
(defn store-received-document-count!
  ...
  (let [resolver-id    (r download.subs/get-resolver-id db extension-id item-namespace :get)
      ...
      (-> db ; A document-count értéke NEM a fogadott dokumentumok mennyiségére utal, hanem
             ...
             (assoc-in [:plugins :item-lister/meta-items extension-id :document-count] document-count)
             ; BUG#7009
             ...
             (assoc-in [:plugins :item-lister/meta-items extension-id :received-count] received-count))))
