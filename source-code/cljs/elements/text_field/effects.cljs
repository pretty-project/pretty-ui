...
(r/reg-event-fx :elements.text-field/empty-field!
  ...
  (fn [{:keys [db]} [_ field-id {:keys [on-empty] :as field-props}]]
      ...
      ; Pl.: A mező on-changed eseménye által indított lekérés {:disabled? true} állapotba
      ...
      ;      megtörténne az on-empty esemény, ami az on-changed eseményhez hasonlóan ismételten
