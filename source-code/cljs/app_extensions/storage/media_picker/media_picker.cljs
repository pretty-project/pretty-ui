...
(ns app-extensions.storage.media-picker
    (:require [x.app-core.api :as a]
              [app-extensions.storage.media-browser :as media-browser]))
...
(defn- get-media-picker-props
  [db [_ picker-id]]
  {:show-order-by-options? (get-in db [:storage :media-picker/meta-items :show-order-by-options?])})
...
(defn- toggle-media-picker-order-by-options!
  [db [_ picker-id]]
  (update-in db [:storage :media-picker/meta-items :show-order-by-options?] not))

(a/reg-event-db :storage/toggle-media-picker-order-by-options! toggle-media-picker-order-by-options!)
...
(defn- media-picker-body
  ...
  [:div {:style {:background "var(--fill-color)"}}
        ...
        [media-picker-directory picker-id "Red directory" "12.0" "3m"]
        [media-picker-directory picker-id "Blue directory" "2.0" "6m"]
        [media-picker-directory picker-id "Pink directory" "2.0" "1d"]
        [media-picker-directory picker-id "Grey directory" "2.0" "3d"]
        [media-picker-directory picker-id "Blue directory" "2.0" "2M"]
        [media-picker-directory picker-id "Brown directory" "2.0" "6M"]
        [media-picker-directory picker-id "White directory" "2.0" "6M"]
        [media-picker-directory picker-id "Black directory" "2.0" "6M"]
        [media-picker-directory picker-id "Yellow directory" "2.0" "6M"]
        [media-picker-directory picker-id "Orange directory" "2.0" "6M"]
        [media-picker-directory picker-id "Crimson directory" "2.0" "6M"]
        [media-picker-directory picker-id "Tourquise directory" "2.0" "6M"]
...
(defn- media-picker-header
  ...
  [:div {:style {:background "var(--fill-color)" :border-bottom "1px solid var(--border-color-highlight)"}}
        [:div {:style {:display "flex"}}
              ...
              [x.app-elements.api/button {:preset :order-by-icon-button :on-click [:storage/toggle-media-picker-order-by-options! picker-id]}]]
...
(defn- media-picker
  ...
  [:div {:style {:margin "48px" :min-width "560px" :border-radius "var(--border-radius-m) var(--border-radius-m)"
                 :overflow "hidden"}}
        [media-picker-header picker-id picker-props]
        [media-picker-body   picker-id picker-props]])
...
(defn view
  ...
  ([picker-id picker-props]
   (let [subscribed-props (a/subscribe [:storage/get-media-picker-props picker-id])]
        (fn [] [media-picker picker-id (merge picker-props @subscribed-props)]))))
