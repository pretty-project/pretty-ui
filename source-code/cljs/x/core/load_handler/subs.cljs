...
(defn get-load-status
  ...
  ; @return (keyword)
  [db _]
  (get-in db [:x.core :load-handler/meta-items :load-status]))
...
(defn get-signal-count
  ...
  ; @return (integer)
  [db _]
  (count (get-in db [:x.core :load-handler/data-items])))
...
(defn get-elapsed-time
  ...
  (let [elapsed-time    (time/elapsed)
        load-started-at (get-in db [:x.core :load-handler/meta-items :load-started-at])]
