...
(ns x.core.process-handler.subs
...
(defn get-process-status
  ...
  ; @return (keyword)
  [db [_ process-id]]
  (let [process-status (get-in db [:x.core :process-handler/data-items process-id :status])]
...
(defn get-process-activity
  ...
  ; @return (keyword)
  [db [_ process-id]]
  (let [process-activity (get-in db [:x.core :process-handler/data-items process-id :activity])]
...
(defn get-process-progress
  ...
  ; @return (percent)
  [db [_ process-id]]
  (let [process-progress (get-in db [:x.core :process-handler/data-items process-id :progress])]
