...
(ns x.gestures.select-handler.subs
...
(defn get-selected-item-ids
  ...
  ; @return (vector)
  [db [_ handler-id]]
  (get-in db [:x.gestures :select-handler/data-items handler-id :selected-items]))
...
(defn select-handler-enabled?
  ...
  ; @return (vector)
  [db [_ handler-id]]
  (let [handler-enabled? (get-in db [:x.gestures :select-handler/data-items handler-id :enabled?])]
