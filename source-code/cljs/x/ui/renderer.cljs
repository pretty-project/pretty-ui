...
; WARNING
; Az x.ui.renderer névtér újraírása szükséges, a Re-Frame adatbázis események
...
(defn- store-element!
  ...
  (let [data-items-key (engine/data-key renderer-id :data-items)
       ...
       (as-> db % (r update-render-log! % renderer-id  element-id :rendered-at)
                  (r x.db/set-item!     % [:x.ui data-items-key element-id] element-props)
                  (r x.db/apply-item!   % [:x.ui data-order-key] vector/move-item-to-last element-id))))
...
(defn- remove-element!
  ...
  (let [data-items-key (engine/data-key renderer-id :data-items)]
       (as-> db % (r update-render-log! % renderer-id  element-id :props-removed-at)
                  (r x.db/remove-item!  % [:x.ui data-items-key element-id]))))
...
(r/reg-event-fx :x.ui/init-renderer!
  ...
  (fn [{:keys [db]} [_ renderer-id renderer-props]]
      (let [data-items-key (engine/data-key renderer-id :data-items)
           ...
           {:db (as-> db % (r x.db/set-item! % [:x.ui data-items-key] {})
                           (r x.db/set-item! % [:x.ui data-order-key] [])
                           (r x.db/set-item! % [:x.ui meta-items-key] renderer-props))})))
...
(r/reg-event-fx :x.ui/destruct-renderer!
  ...
  (fn [{:keys [db]} [_ renderer-id _]]
      {:dispatch-if [(r renderer-require-error?   db renderer-id)
                     [:x.ui/renderer-unmounted-illegal renderer-id]]}))
...
(r/reg-event-fx :x.ui/render-element-animated!
  ...
  (fn [{:keys [db]} [_ renderer-id element-id element-props]]
      {:db             (r store-element! db renderer-id element-id element-props)
       :dispatch-later [{:ms REVEAL-ANIMATION-TIMEOUT :dispatch [:x.ui/rendering-ended renderer-id]}]}))
...
(r/reg-event-fx :x.ui/renderer-unmounted-illegal
  ; WARNING! NON-PUBLIC! DO NOT USE!
  ;
  ; @param (keyword) renderer-id
  (fn [cofx [_ renderer-id]]
      [:x.core/error-catched {:error ILLEGAL-UNMOUNTING-ERROR :cofx cofx}]))
