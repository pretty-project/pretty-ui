...
(defn get-ordered-routes
  ...
  ;   [...]]
  [db _]
  (get-in db [:x.router :route-handler/ordered-routes]))
...
(defn route-restricted?
  ...
  ; @return (boolean)
  [db [_ route-id]]
  (let [route-restricted? (get-in db [:x.router :route-handler/client-routes route-id :restricted?])]
...
(defn get-current-route-string
  ...
  ; @return (string)
  [db _]
  (get-in db [:x.router :route-handler/meta-items :route-string]))
...
(defn get-current-route-id
  ...
  ; @return (keyword)
  [db _]
  (get-in db [:x.router :route-handler/meta-items :route-id]))
...
(defn get-current-route-path
  ...
  ; @return (string)
  [db _]
  (get-in db [:x.router :route-handler/meta-items :route-path]))
...
(defn get-current-route-template
  ...
  (let [current-route-id (r get-current-route-id db)]
       (get-in db [:x.router :route-handler/client-routes current-route-id :route-template])))
...
(defn get-current-js-build
  ...
  [db _]
  (let [current-route-id (r get-current-route-id db)]
       (get-in db [:x.router :route-handler/client-routes current-route-id :js-build])))
...
(defn get-current-route-path-params
  ...
  ; @return (map)
  [db _]
  (get-in db [:x.router :route-handler/meta-items :route-path-params]))
...
(defn get-current-route-path-param
  ...
  ; @return (string)
  [db [_ param-key]]
  (get-in db [:x.router :route-handler/meta-items :route-path-params param-key]))
...
(defn get-current-route-query-params
  ...
  ; @return (map)
  [db _]
  (get-in db [:x.router :route-handler/meta-items :route-query-params]))
...
(defn get-current-route-query-param
  ...
  ; @return (string)
  [db [_ param-key]]
  (get-in db [:x.router :route-handler/meta-items :route-query-params param-key]))
...
(defn get-history
  ...
  [db _]
  (get-in db [:x.router :route-handler/meta-items :history]))
...
; WARNING! NON-PUBLIC! DO NOT USE!
(r/reg-sub :x.router/route-template-exists? route-template-exists?)

; WARNING! NON-PUBLIC! DO NOT USE!
(r/reg-sub :x.router/match-route-id match-route-id)

; @usage
;  [:x.router/get-current-route-string]
(r/reg-sub :x.router/get-current-route-string get-current-route-string)

; @usage
;  [:x.router/get-current-route-id]
(r/reg-sub :x.router/get-current-route-id get-current-route-id)

; @usage
;  [:x.router/get-current-route-path]
(r/reg-sub :x.router/get-current-route-path get-current-route-path)

; @usage
;  [:x.router/get-current-route-template]
(r/reg-sub :x.router/get-current-route-template get-current-route-template)

; @usage
;  [:x.router/get-current-js-build]
(r/reg-sub :x.router/get-current-js-build get-current-js-build)

; @usage
;  [:x.router/get-current-route-path-params]
(r/reg-sub :x.router/get-current-route-path-params get-current-route-path-params)

; @usage
;  [:x.router/get-current-route-path-param :my-param]
(r/reg-sub :x.router/get-current-route-path-param get-current-route-path-param)

; @usage
;  [:x.router/current-route-path-param? :my-param "My value"]
(r/reg-sub :x.router/current-route-path-param? current-route-path-param?)

; @usage
;  [:x.router/get-current-route-query-param]
(r/reg-sub :x.router/get-current-route-query-params get-current-route-query-params)

; @usage
;  [:x.router/get-current-route-query-param :my-param]
(r/reg-sub :x.router/get-current-route-query-param get-current-route-query-param)

; @usage
;  [:x.router/current-route-query-param? :my-param "My value"]
(r/reg-sub :x.router/current-route-query-param? current-route-query-param?)

; @usage
;  [:x.router/get-current-route-fragment]
...
; @usage
;  [:x.router/at-home?]
(r/reg-sub :x.router/at-home? at-home?)

; @usage
;  [:x.router/home-next-door?]
(r/reg-sub :x.router/home-next-door? home-next-door?)
